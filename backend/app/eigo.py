...
DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "eigo"
...
def max_grade_for_step(step: int) -> int:
    step = min(max(step, 1), EIGO_MAX_STEP)
    if step <= 2:
        return 3
    if step == 3:
        return 4
...
def _load_bank(kind: str) -> list[dict]:
    ...
    if filename is None:
        raise ValueError(f"unknown eigo kind: {kind}")
...
def pick_ten(kind: str, step: int = 1) -> list[GeneratedQuestion]:
    ...
    while len(out) < 10:
        out.append(_one(random.choice(pool)))
    return out
